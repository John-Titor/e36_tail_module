...
//
uint8_t
iso_tp_send(struct iso_tp_request *req)
{
    tp_frame_t  tf;
    uint8_t     i;
    uint8_t     ret;
    ...
    if (req->len <= sizeof(tf.single.data)) {

        // pack a SINGLE_FRAME message
        tf.single.recipient = req->recipient;
        tf.single.type = TP_SINGLE_FRAME;
        tf.single.len = req->len;
        for (i = 0; i < sizeof(tf.single.data); i++) {
            tf.single.data[i] = (i < req->len) ? req->buf[i] : 0;
        }
    ...
    // otherwise pack a first frame and set up to continue
        ...
        tf.first.recipient = req->recipient;
        tf.first.type = TP_FIRST_FRAME;
        tf.first.len_hi = 0;
        tf.first.len_lo = req->len;
        for (i = 0; i < sizeof(tf.first.data); i++) {
            tf.first.data[i] = req->buf[i];
        }

        // set up the rest of the request so that we can continue
        // it once a flow control message arrives
        tp_tx.sender = req->sender;
        tp_tx.recipient = req->recipient;
        tp_tx.resid = req->len - sizeof(tf.first.data);
        tp_tx.sequence = 1;
        timer_register(tp_tx.timeout);
        timer_reset(tp_tx.timeout, req->timeout_ms);
        tp_tx.buf = req->buf + sizeof(tf.first.data);
...
//
uint8_t
iso_tp_recv(struct iso_tp_request *req)
{
    uint8_t ret = iso_tp_rx_done();
    if (ret == ISO_TP_BUSY) {
        return ret;
    }

    tp_rx.sender = req->sender;
    tp_rx.recipient = req->recipient;
    tp_rx.resid = req->len;
    tp_tx.buf = req->buf;
    timer_register(tp_rx.timeout);
    timer_reset(tp_rx.timeout, req->timeout_ms);
}
...
// Handle a presumed ISO-TP frame
//
void
